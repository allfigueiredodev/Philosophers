...
/*   Updated: 2024/02/14 19:51:57 by aperis-p         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (lock_int_return(&head->philo.philo_mtx,
			&head->philo.meals_ate) >= data->args.meals_must_eat)
		data->eat_score++;
...
void	check_philo_health(t_data *data)
{
	...
	while (!data->health_status)
	{
		...
		if (current - lock_long_return(&head->philo.philo_mtx,
			...
			&& lock_state_return(&head->philo.philo_mtx,
